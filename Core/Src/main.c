...
// Luke Fadel
// June 15, 2026
// For X-NUCLEO-GFX01M2 (uses ILI9341V driver)
// using NUCLEO-F091RC development board
...
uint8_t confirmation[] = "\n\rCommand was received\r\n";
// on serial command receive
...
// memory access control (row/col order, RGB order) - instruction 36h
	LCD_CMD(&hspi1, 0x36);      // MADCTL
// bgr enabled, all other bits disabled (00001000)
	uint8_t madctl = 0x8;
...
static void MX_GPIO_Init(void) {
	...
	HAL_GPIO_WritePin(GPIOA, DISP_RESET_Pin | LCD_NC_Pin, GPIO_PIN_RESET);
	...
	/*Configure GPIO pins : DISP_RESET_Pin LCD_NC_Pin */
	GPIO_InitStruct.Pin = DISP_RESET_Pin | LCD_NC_Pin;
